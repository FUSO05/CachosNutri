...
const { hasTestAccount, TEST_EMAIL, TEST_PASSWORD, login, deleteAllClients } = require('./helpers');
...
test.describe('Importação de dados locais pré-existentes', () => {
  ...
  test('oferece importar dados do localStorage quando a conta remota está vazia', async ({ page, browser }) => {
    ...
    await page2.waitForURL('**/app.html', { timeout: 20000 });
    await page2.waitForSelector('#pg-clients', { state: 'visible' });
